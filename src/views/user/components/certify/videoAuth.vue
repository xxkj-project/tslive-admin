 ...
 * @LastEditTime: 2020-09-29 15:18:24
 * @Desc: 实名认证 - 身份认证list
-->
<template>
  <div class="identity-auth">
    <el-table :data="list" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelChange">
      ...
      <el-table-column :key="Math.random()" prop="photoPositive" label="证件照" min-width="100">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-view" @click="findIdCard(scope.row)">查看</el-button>
        </template>
      </el-table-column>
      <el-table-column :key="Math.random()" prop="videoUrl" label="视频认证" min-width="100">
        <template slot-scope="scope">
...
export default {
  ...
  watch: {
    isFinished(val) {
      console.log('-init-multiple-list-', val)
