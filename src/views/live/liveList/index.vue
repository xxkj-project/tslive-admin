 ...
 * @LastEditTime: 2020-11-06 17:23:10
 * @Desc: 直播数据 - 直播时长
-->
<template>
  <div class="userList">
...
import { findAllLiveUserInfo } from '@/api/live'
...
export default {
  ...
  methods: {
    ...
    handleSearch(data) {
      ...
      const { infoSearch, type, times, guildId } = data
      let startTime = times ? times[0] : ''
      let endTime = times ? times[1] : ''
      this.searchData = { infoSearch, type, startTime, endTime, guildId }
      this.getInitData()
    },
    ...
    handleExport() {
      ...
      exportExcel('liveList', this.searchData)
    },
    sizeChange(pageSize) {
    ...
    getData() {
      ...
      findAllLiveUserInfo(params).then(res => {
        // console.log('-res-', res)
