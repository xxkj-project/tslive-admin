 ...
 * @LastEditTime: 2020-11-24 14:41:25
 * @Desc: 视频上传 -- 公共组件
-->
<template>
...
export default {
  ...
  computed: {
    videoList: {
      get() {
        // console.log('-video-get-', this.value)
        return this.value
      },
      set(val) {
        // console.log('-video-set-', val)
  ...
  methods: {
    ...
    onError(err, file, fileList) {
      console.log('-upload-err-', err, file, fileList)
    },
    // 超限
    onExceed(file, fileList) {
      console.log('-上传超限-', file, fileList)
      ...
      uploadImgOrVideo(formData)
        .then(res => {
          if (res.code === 200) {
            ...
            // TODO:push操作不能触发计算属性中videoList中set方法
