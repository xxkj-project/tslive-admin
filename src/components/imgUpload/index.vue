...
export default {
  ...
  computed: {
    ...
    imgList: {
      get() {
        // console.log('-img-get-', this.value)
        return this.value
      },
      set(val) {
        // console.log('-img-set-', val)
  ...
  methods: {
    ...
    handleHttpRequest(a) {
      console.log('-handle-http-request-', a.file)
      ...
      uploadImgOrVideo(formData)
        .then(res => {
          ...
          if (res.code === 200) {
            ...
            // push操作不能触发 计算属性中imgList的 set 方法
            // this.imgList.push(res.data[0].picUrl)
            this.imgList = [...this.imgList, res.data[0].picUrl]
