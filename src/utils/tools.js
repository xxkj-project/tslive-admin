 ...
 * @LastEditTime: 2020-12-18 15:50:32
 * @Desc: 工具函数
 */
 ...
 */
function getWeekAgo() {
  let currTs = new Date(formatDate(new Date(), '{y}/{m}/{d}')).getTime()
 ...
 * @returns {array} ['2021-02-23 00:00:00','2021-03-01 23:59:59]
 */
function getRecentlyDays(days = 7) {
  let ts = new Date(formatDate(new Date(), '{y}/{m}/{d}')).getTime()
  let startTime = formatDate(ts - 24 * 60 * 60 * (days - 1) * 1000)
  let endTime = formatDate(ts + 24 * 60 * 60 * 1000 - 1000)
...
/**
 * 剩余多少时间
 * @param {any} endTime 结束时间(格式可以为date对象/毫秒级时间戳/秒级时间戳)
 */
function getSurplusTime(endTime, startTime, format) {
  format = format || 'd:h:m:s'
  let startTs
  let endTs
  if (typeof startTime === 'number') {
    if (('' + startTime).length === 10) startTs = +startTime * 1000
  } else {
    startTs = new Date(startTime).getTime()
  }
  if (typeof endTime === 'number') {
    if (('' + endTime).length === 10) endTs = +endTime * 1000
  } else {
    endTs = new Date(endTime).getTime()
  }

  let time = endTs - startTs
  // console.log('-slurplus-', time)
  ...
  let result
  if (formatObj.d === 0) {
    result = `${formatObj.h}时${formatObj.m}分`
  } else {
    result = `${formatObj.d}天${formatObj.h}时${formatObj.m}分`
  }
  return result
}
...
/**
 * 对象参数序列化(在Qs.stringify(obj)的基础上增加了参数转码)
...
/**
 * 函数防抖(原理：每次执行都把上一次的延时器清除掉，直到等待的时间内没有再执行，才会执行目标函数)
...
/**
 * 函数节流(多次触发，规定时间内只会执行一次目标函数，且一定会执行)
 * @param {function} fn 函数
...
function throttle(fn, t) {
  ...
  return function() {
    let args = arguments
    if (firstTime) {
      // 该判断只最开始执行一次，后面就不会再执行，且变量firstTime会常驻内存中不会被释放掉
      fn.apply(this, args)
      firstTime = false
      return
    }
